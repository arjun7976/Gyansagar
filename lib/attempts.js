...
export async function score(a, test, status = "submitted") {
  ...
  if (a.selectedQuestions && a.selectedQuestions.length > 0) {
    ...
    const sqMap = {};
    a.selectedQuestions.forEach(sq => { sqMap[sq.questionId.toString()] = sq; });
    qs = ids.map(id => ({
      _id: id,
      correctAnswer: sqMap[id.toString()]?.correctAnswer || qMap[id.toString()]?.correctAnswer,
      marks: sqMap[id.toString()]?.marks ?? qMap[id.toString()]?.marks ?? 1
    }));
  } else {

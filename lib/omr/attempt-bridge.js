...
export async function omrQuestionMap(testId) {
  ...
  const rows = await TestQuestion.find({ testId })
  ...
  const mapping = rows.map((row, index) => {
    ...
    if (!question) {
      throw new OMRTestValidationError(`Question mapping failed at OMR question ${questionNumber}. Question document not found or inactive.`, {
        questionNumber,

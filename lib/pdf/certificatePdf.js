...
export async function generateCertificatePDF(certificate, student, test) {
  ...
  if (certificate.type === "monthly") {
    doc.text("for outstanding overall performance as a", 148.5, 115, { align: "center" });
  } else {
    doc.text("for successfully completing", 148.5, 115, { align: "center" });
  }

  // Test name or Monthly Champion title
  doc.setTextColor(37, 99, 235);
  doc.setFontSize(24);
  doc.setFont("helvetica", "bold");
  if (certificate.type === "monthly") {
    doc.text(`MONTHLY CHAMPION - ${certificate.monthYear?.toUpperCase()}`, 148.5, 130, { align: "center" });
  } else {
    doc.text(test?.title?.toUpperCase() || "CERTIFICATE", 148.5, 130, { align: "center" });
  }
